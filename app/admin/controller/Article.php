...
class Article extends Permissions
{
    ...
    {        
		...
		if (isset($post['week_top']) and ($post['week_top'] == 1 or $post['week_top'] === '0')) {
            $where['week_top'] = $post['week_top'];
        }
        ...
        $lists = $model->alias('aa')
                ->field('aa.*, a.nickname, att.filepath as thumb')
    ...
    public function is_top()
    {
        if($this->request->isPost()){
            ...
            if(false == Db::name('article')->where('id',$post['id'])->update(['is_top'=>$post['is_top']])) {
                ...
                addlog($post['id']);//写入日志
                return $this->success('推荐设置成功','admin/article/index');
            }
        }
    }
    ...
    public function orders()
    {
        if($this->request->isPost()) {
            ...
            foreach ($post['orders'] as $k => $val) {
                $order = Db::name('article')->where('id',$k)->value('orders');
                if($order != $val) {
                    if(false == Db::name('article')->where('id',$k)->update(['orders'=>$val])) {
            ...
            addlog();//写入日志
            return $this->success('排序成功更新'.$i.'个数据', 'admin/article/index');
        }
    }
    ...
    public function deleteAll()
    {
        if($this->request->isAjax()) {           
            ...
            //批量删除的 条数
            $rows = Db::name('article')->delete($data);
            #dump( Db::getLastSql());
            $num = count($data) - $rows; //提交的条数与实现删除的条数
            if(0 == $num) {
                addlog(implode(",",$data));//写入日志
                return $this->success('成功删除'.$rows.'笔数据', 'admin/article/index');
            } else {
