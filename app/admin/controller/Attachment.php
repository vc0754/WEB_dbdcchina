...
class Attachment extends Permissions
{
    public function index()
    {
        ...
        $attachment = $model->alias('aa')
                ->field('aa.*, a.nickname')
                ->join('__ADMIN__ a', 'a.id=aa.user_id','left')
                ->where($where)
                ->order('aa.id desc')
    ...
    public function deleteAll()
    {
    	if($this->request->isAjax()) {
    		...
    		$attachments = Db::name('attachment')->where('id','in',$data)->column('filepath');
            ...
            //批量删除的 条数
            $rows = Db::name('attachment')->delete($data);
            if(false == $rows){
                ...
                if(0 == $num) {
                    ...
                    return $this->success('成功删除'.$rows.'笔数据','admin/attachment/index');
                }else{
    ...
    public function upload($module='admin',$use='attachment')
    {
        ...
        $info = $file->validate(['size'=>config('web_config.file_size'),'ext'=>config('web_config.file_type')])->rule('date')->move(ROOT_PATH . 'public' . DS . 'uploads' . DS . $module . DS . $use);
        ...
        if($info) {
            ...
            $data['filepath'] = str_replace('\\', '/', $data['filepath']); // oo 路径盘符转换为/　
